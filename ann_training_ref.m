function ann_training_ref(learningRate, noHiddenNeurons, noEpochs)
...
for epoch=1:noEpochs
	...
	if (improvement > 0)	
		...
		if(testCorrectness(epoch) > 0.8742)
			...
			xlswrite(strcat("CFMX_E",int2str(epoch),"_R", num2str(learningRate), "_L", num2str(noHiddenNeurons(1)), "_", num2str(noHiddenNeurons(2)), ".xlsx"),cfmx,'CFMX','');
		else
...
xlswrite(strcat("TRAINING_REPORT", "_R", num2str(learningRate), "_L", num2str(noHiddenNeurons(1)), "_", num2str(noHiddenNeurons(2)), ".xlsx"),trReport,'Training Report','');
end
